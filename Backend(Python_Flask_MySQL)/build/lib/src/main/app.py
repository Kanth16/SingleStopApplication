...
def joinss():
    ...
    if request.method=='POST':
        ...
        cur = mysql.connection.cursor()
        sql = ("select column_name from information_schema.columns where table_name=" + "'" + tablename1 + "'")
        cur.execute(sql)
        fetchdata=cur.fetchall()
        for i in fetchdata:
            selectedcolumns1.append(i['COLUMN_NAME'])
        tablename2=joindata['tablename2']
        sql = ("select column_name from information_schema.columns where table_name=" + "'" + tablename2 + "'")
        cur.execute(sql)
        fetchdata=cur.fetchall()
        for i in fetchdata:
...
@app.route('/joincol/<string:tablename1>/<string:tablename2>')
def joinscol(tablename1,tablename2):
    cur = mysql.connection.cursor()
    sql="select distinct(column_name) from information_schema.columns where column_name in (select column_name from information_schema.columns where table_name = "+"'"+tablename1+"') and column_name in (select column_name from information_schema.columns where table_name ="+"'"+tablename2+"')"
    cur.execute(sql)
    fetchdata=cur.fetchall()
    response=jsonify(fetchdata)
    return response

@app.route('/allcol/<string:tablename1>/<string:tablename2>')
def allcols(tablename1,tablename2):
    cur = mysql.connection.cursor()
    sql="select distinct(column_name) from information_schema.columns where table_name = "+"'"+tablename1+"' or table_name ="+"'"+tablename2+"'"
    cur.execute(sql)
    fetchdata=cur.fetchall()
    response=jsonify(fetchdata)
    return response
    ...
    if request.method == 'POST':
        ...
        if func == 'MIN' or func == 'MAX' or func == 'AVG' or func == 'SUM' or func == 'COUNT': 
            ...
            for i in selectedcolumns:
                ...
                columns.append("".join(a))
            selectedcolumns = ",".join(columns)
            sql="SELECT "+selectedcolumns+","+func+"("+aggcolumn+") over( PARTITION BY "+partitioncol+" ORDER BY "+orderbycol+")  from "+tablename
            cur.execute(sql)
            fetchdata=cur.fetchall()
            response=jsonify(fetchdata)
...
def queryexec():
    ...
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        if comparator == "no" :
            # if func == 'GROUP BY' and having == "yes":
            #     print("groupby and having w/o count, sum or avg")
            #     sql="SELECT " + groupcolumnname + ", " + aggcolumn +" FROM "+tablename+" GROUP BY "+groupcolumnname+" having "+aggcolumn+" = (SELECT "+agg+"("+aggcolumn+") from "+tablename+")"
            #     print(sql)
            #     cur.execute(sql)
            #     fetchdata=cur.fetchall()
            #     response=jsonify(fetchdata)
